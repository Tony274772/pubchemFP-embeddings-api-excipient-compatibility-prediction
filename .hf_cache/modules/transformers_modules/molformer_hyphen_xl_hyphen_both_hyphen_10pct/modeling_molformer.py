...
class MolformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if not torch.equal(attention_mask, per_query_extended.expand_as(attention_mask)):
                raise ValueError(
...
class MolformerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, MolformerRotaryEmbedding):
            inv_freq = 1.0 / (module.base ** (torch.arange(0, module.dim, 2).float() / module.dim))
